...
function processLoadingDirective(component, el, directive) {
    ...
    if (directives.get('target')) {
        ...
        if (target.params.length > 0) {
            ...
            // axm:target overrides any automatic loading scoping we do.
        ...
        const nonActionOrModelLiveaxmDirectives = [
            'init',
            'dirty',
            'offline',
            'target',
        ...
        actionNames = directives
            .all()
            .filter(i => !nonActionOrModelLiveaxmDirectives.includes(i.type))
            .map(i => i.method)
...
function doAndSetCallbackOnElToUndo(el, directive, doCallback, undoCallback) {
    ...
    if (directive.modifiers.includes('delay')) {
        ...
        let timeout = setTimeout(() => {
            doCallback()
            el.__axm_on_finish_loading.push(() => undoCallback())
        }, duration)

        el.__axm_on_finish_loading.push(() => clearTimeout(timeout))
    } else {
        doCallback()
        el.__axm_on_finish_loading.push(() => undoCallback())
    }
...
function endLoading(els) {
    els.forEach(({ el }) => {
        while (el.__axm_on_finish_loading.length > 0) {
            el.__axm_on_finish_loading.shift()()
        }
    })
}
