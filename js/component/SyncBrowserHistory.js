...
export default function () {
    ...
    LiveaxmStateManager.clearState()
    ...
    store.registerHook('component.initialized', component => {
        ...
        setTimeout(() => {
            ...
            LiveaxmStateManager.replaceState(url, response, component)
    ...
    store.registerHook('message.processed', (message, component) => {
        ...
        if ('path' in effects && effects.path !== window.location.href) {
            ...
            LiveaxmStateManager.pushState(url, response, component)
            ...
            if (componentIdsThatAreWritingToHistoryState.has(component.id)) {
                LiveaxmStateManager.replaceState(window.location.href, response, component)
            }
        }
    })

    window.addEventListener('popstate', event => {
        if (LiveaxmStateManager.missingState(event)) return

        LiveaxmStateManager.replayResponses(event, (response, component) => {
    ...
    function normalizeResponse(response, component) {
        ...
        // Sometimes Liveaxm doesn't return html from the server to save on bandwidth.
    ...
    store.registerHook('element.updating', (from, to, component) => {
        ...
        if (from.getAttribute('axm:id') === component.id) {
    ...
    store.registerHook('element.updated', (node, component) => {
        ...
        if (component.lastKnownDomId) {
            ...
            if (node.getAttribute('axm:id') !== component.lastKnownDomId) {
                // If so, we need to change this ID globally everwhere it's referenced.
                store.changeComponentId(component, node.getAttribute('axm:id'))
            }
...
let LiveaxmStateManager = {
    ...
    updateState(method, url, response, component) {
        ...
        let fullstateObject = Object.assign(history.state || {}, { liveaxm: stateArray })
    ...
    replayResponses(event, callback) {
        if (!event.state.liveaxm) return

        let state = typeof event.state.liveaxm === 'string'
            ? new LiveaxmState(this.getFromSession(event.state.liveaxm))
            : new LiveaxmState(event.state.liveaxm)

        state.replayResponses(callback)
    },

    currentState() {
        if (!history.state) return new LiveaxmState
        if (!history.state.liveaxm) return new LiveaxmState

        let state = typeof history.state.liveaxm === 'string'
            ? new LiveaxmState(this.getFromSession(history.state.liveaxm))
            : new LiveaxmState(history.state.liveaxm)

        return state
    },

    missingState(event) {
        return !(event.state && event.state.liveaxm)
    },

    clearState() {
        // This is to prevent exponentially increasing the size of our state on page refresh.
        if (window.history.state) window.history.state.liveaxm = (new LiveaxmState).toStateArray();
    },

    storeInSession(value) {
        let key = 'liveaxm:' + (new Date).getTime()
    ...
    tryToStoreInSession(key, value) {
        ...
        try {
            ...
            let oldestTimestamp = Object.keys(sessionStorage)
                .map(key => Number(key.replace('liveaxm:', '')))
                .sort()
                .shift()

            if (!oldestTimestamp) return

            sessionStorage.removeItem('liveaxm:' + oldestTimestamp)
...
class LiveaxmState {
    ...
    findComponentBySignature(signature) {
        ...
        return sameNamedComponents[componentIndex] || sameNamedComponents[0] || console.warn(`Liveaxm: couldn't find component on page: ${componentName}`)
    }
